...
const withDotLottie = (config) => {
  ...
  return withPodfileProperties(config, (podfileConfig) => {
    ...
    // The iOS player ships as the LottieFiles-dotLottie-iOS CocoaPods pod, which
    // vendors DotLottiePlayer.xcframework as a dynamic framework. Default to
    // dynamic linkage so it is embedded as a single shared image, unless the app
